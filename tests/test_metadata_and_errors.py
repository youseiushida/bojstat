...
def test_metadata_filter() -> None:
    csv_text = "\n".join(
        [
            ...
            "SERIES_CODE,NAME_OF_TIME_SERIES_J,UNIT_J,FREQUENCY,CATEGORY_J,LAYER1,LAYER2,LAYER3,LAYER4,LAYER5,START_OF_THE_TIME_SERIES,END_OF_THE_TIME_SERIES,LAST_UPDATE,NOTES_J",
            "FXERD01,ドル・円,円,DAILY,外国為替,1,1,0,0,0,19990101,20251231,20251001,備考",
            "FXERS01,ユーロ・円,円,DAILY,外国為替,1,2,0,0,0,19990101,20251231,20251001,",
            "IR01,無担保コール,％,DAILY,金利,2,1,0,0,0,20000101,20251231,20251001,",
        ]
    )
    ...
    # カテゴリで絞り込み
    fx = frame.filter(lambda r: r.category == "外国為替")
    assert fx.series_codes == ["FXERD01", "FXERS01"]

    # 複合条件
    hit = frame.filter(lambda r: r.layer1 == "1" and r.layer2 == "2")
    assert hit.series_codes == ["FXERS01"]

    # 該当なし
    empty = frame.filter(lambda r: r.unit == "トン")
    assert empty.series_codes == []

    # metaが保持されること
    assert fx.meta.status == frame.meta.status
